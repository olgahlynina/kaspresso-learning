...
class Schedule {
    ...
    private fun addDay(dayOfWeek: Days, fnc: () -> Unit) {
    ...
    operator fun String.rangeTo(endString: String): Pair<LocalTime, LocalTime> {
    ...
    infix fun Pair<LocalTime, LocalTime>.schedule(lesson: String) {
        val activeDay = currentDay ?: throw IllegalStateException("Уроки должны объявляться внутри блока дня!")
        val entity = ScheduleEntity(lesson, this.first, this.second)
        addSchedule(activeDay, entity)
    }
}
